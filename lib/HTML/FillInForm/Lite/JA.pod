...
オブジェクト，アクセサを持つオブジェクト，前述のいずれかからなる
配列リファレンスを渡すことが出来ます。

I<form_data>が手続きに基づくもの（つまりハッシュリファレンス以外のケース）では，
その手続きはリストコンテキストで呼ばれます。したがって，フィールドに手を付けたくない場合，C<undef>ではなく空リストC<()>を返さなければいけません。
